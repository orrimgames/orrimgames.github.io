// pacing audit 3: minimum-viable-raiding - two barracks, raid at 10 troops with fire+rally. Based on pacing2. - first raid through day 3. Scripted, sim-stepped 0.05s, chunked.
 ...
 await p.evaluate(()=>{
  ...
  P.think=function(dt){
   ...
   if(next){
    ...
    if(S.war.troops>=10){ P.raids++; P.ev.push({t:Math.round(P.t),ev:'raid '+next.id+' (tier '+next.tier+', '+S.war.troops+' troops)'});
