// pacing audit 7: tier-4 vs TREATY - two barracks, 16 troops, fire+rally, Cold Peace legion active (probe cheat: treaty pre-signed). Based on pacing4. - first raid through day 3. Scripted, sim-stepped 0.05s, chunked.
...
(async()=>{
 ...
 await p.evaluate(()=>{
  ...
  P.think=function(dt){
   ...
   const gi=goalIdx();
   if(!S.war.treaty){ S.war.treaty=true; P.ev.push({t:Math.round(P.t),ev:'treaty ACTIVE (forced for probe)'}); }
   if(S.war.treaty&&S.res.gold<10){ S.res.gold=20; }   // probe cheat: keep the 4g dawn tithe paid so the treaty survives
   ...
   if(next){
    ...
    if(S.war.troops>=16){ P.raids++; P.ev.push({t:Math.round(P.t),ev:'raid '+next.id+' (tier '+next.tier+', '+S.war.troops+' troops)'});
