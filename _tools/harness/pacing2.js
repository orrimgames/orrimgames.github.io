 ...
 await p.evaluate(()=>{
  ...
  P.think=function(dt){
   ...
   if(S.mode==='battle'){ if(S.war.result){ if(P._resT===undefined) P._resT=P.t;
     if(P.t-P._resT>2){ leaveBattle(); P._resT=undefined; P.ev.push({t:Math.round(P.t),ev:'result dismissed'}); return 'dismiss result'; } return 'reading result'; }
   ...
   if(next){
    ...
    if(S.war.troops>=next.cost+2){ P.raids++; P.ev.push({t:Math.round(P.t),ev:'raid '+next.id+' (tier '+next.tier+', '+S.war.troops+' troops)'});
      const b0=S.war.banners.length; startBattle(next); P._raidB0=b0; return 'battle'; }
